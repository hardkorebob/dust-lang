...
Widget ui_button;
(ui_button.id_i = 101);
strcpy(ui_button.name_c, "Login Button");
printf("Widget created with ID %d and Name '%s'.\n\n", ui_button.id_i, ui_button.name_c);
printf("2. Assigning function pointers...\n");
(ui_button.on_destroy_fp = &widget_destroy);
(ui_button.on_update_fp = &widget_update);
(ui_button.on_event_fp = &widget_event);
printf("Assignments successful.\n\n");
printf("3. Calling functions via pointers...\n");
ui_button.on_event_fp(&ui_button);
ui_button.on_update_fp(0.016);
ui_button.on_destroy_fp();
